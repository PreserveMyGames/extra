...
class AltchaService
{
    ...
    {
        $max = (int) $this->settings->get('preservemygames-altcha.cost', 50000);
    ...
    private function ensureHmacSecret(): string
    {
        ...
        $secret = (string) $this->settings->get('preservemygames-altcha.hmac_secret', '');
        if ($secret !== '') {
            return $secret;
        }

        $secret = bin2hex(random_bytes(32));
        $this->settings->set('preservemygames-altcha.hmac_secret', $secret);
