...
class ModerationActions
{
    ...
    public function applyForPost(CommentPost $post, User $author, SpamVerdict $verdict): void
    {
        if (! $verdict->isSpam || $verdict->confidence < $this->minConfidence()) {
            $this->logger->info('preservemygames-spam-protection: post clean or below threshold', [
                'post_id' => $post->id,
                'user_id' => $author->id,
        ...
        $this->logger->info('preservemygames-spam-protection: moderated post', [
            'post_id' => $post->id,
            'user_id' => $author->id,
            'confidence' => $verdict->confidence,
            'actions' => $actions,
    ...
    public function applyForDiscussion(Discussion $discussion, User $author, SpamVerdict $verdict): void
    {
        ...
        $this->logger->info('preservemygames-spam-protection: moderated discussion', [
    ...
    public function applyForUser(User $user, SpamVerdict $verdict): void
    {
        ...
        $this->logger->info('preservemygames-spam-protection: moderated user', [
            'user_id' => $user->id,
            'confidence' => $verdict->confidence,
            'actions' => $actions,
            'reason' => $verdict->reason,
    ...
    private function hidePost(Post $post): void
    {
        try {
            ...
            $this->logger->warning('preservemygames-spam-protection: hide post failed', [
                'post_id' => $post->id,
                'error' => $e->getMessage(),
            ]);
        }
    ...
    private function hideDiscussion(Discussion $discussion): void
    {
        try {
            ...
            $this->logger->warning('preservemygames-spam-protection: hide discussion failed', [
                'discussion_id' => $discussion->id,
                'error' => $e->getMessage(),
            ]);
        }
    ...
    private function lockDiscussion(Discussion $discussion): void
    {
        ...
        try {
            ...
            $this->logger->warning('preservemygames-spam-protection: lock discussion failed', [
                'discussion_id' => $discussion->id,
                'error' => $e->getMessage(),
            ]);
        }
    ...
    private function suspendUser(User $user, string $reason): void
    {
        ...
        try {
            $days = max(1, (int) $this->settings->get('preservemygames-spam-protection.suspend_days', 30));
            ...
            $this->logger->warning('preservemygames-spam-protection: suspend user failed', [
                'user_id' => $user->id,
                'error' => $e->getMessage(),
            ]);
        }
    ...
    {
        return max(0, min(100, (int) $this->settings->get('preservemygames-spam-protection.min_confidence', 70)));
    }

    private function settingEnabled(string $key): bool
    {
        return (bool) (int) $this->settings->get('preservemygames-spam-protection.'.$key, 1);
    }
}
