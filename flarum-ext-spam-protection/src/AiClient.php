...
class AiClient
{
    ...
    {
        return (bool) (int) $this->settings->get('preservemygames-spam-protection.enabled', 0)
            && $this->isConfigured();
    }
    ...
    {
        if (! $this->isEnabled()) {
            $this->logger->warning('preservemygames-spam-protection: classify skipped (disabled or unconfigured)', [
                'kind' => $kind,
            ]);
        ...
        try {
            ...
            $this->logger->info('preservemygames-spam-protection: AI verdict', [
                'kind' => $kind,
            ...
            $this->logger->warning('preservemygames-spam-protection: AI request failed', [
                'kind' => $kind,
                'error' => $e->getMessage(),
            ]);
...
PROMPT;
    ...
    {
        $setting = trim((string) $this->settings->get('preservemygames-spam-protection.api_key', ''));
        if ($setting !== '') {
            return $setting;
        }
    ...
    {
        $setting = trim((string) $this->settings->get('preservemygames-spam-protection.base_url', ''));
        if ($setting !== '') {
    ...
    {
        $setting = trim((string) $this->settings->get('preservemygames-spam-protection.model', ''));
        if ($setting !== '') {
            return $setting;
        }
    ...
    {
        return (bool) (int) $this->settings->get('preservemygames-spam-protection.fail_open', 1);
    }
}
