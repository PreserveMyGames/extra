...
class SpamMonitor
{
    ...
    public function shouldMonitorPosts(): bool
    {
        return $this->ai->isEnabled()
            && (bool) (int) $this->settings->get('preservemygames-spam-protection.monitor_posts', 1);
    }

    public function shouldMonitorEdits(): bool
    {
        return $this->shouldMonitorPosts()
            && (bool) (int) $this->settings->get('preservemygames-spam-protection.monitor_edits', 1);
    }

    public function shouldMonitorNewUsers(): bool
    {
        return $this->ai->isEnabled()
            && (bool) (int) $this->settings->get('preservemygames-spam-protection.monitor_new_users', 1);
    }
    ...
    public function reviewPost(CommentPost $post, string $kind = 'post'): SpamVerdict
    {
        ...
        try {
            ...
            $this->logger->error('preservemygames-spam-protection: reviewPost failed', [
                'post_id' => $post->id,
                'error' => $e->getMessage(),
            ]);
    ...
    public function reviewUser(User $user, string $kind = 'new_user'): SpamVerdict
    {
        ...
        try {
            ...
            $this->logger->error('preservemygames-spam-protection: reviewUser failed', [
                'user_id' => $user->id,
                'error' => $e->getMessage(),
            ]);
