...
app.initializers.add('preservemygames-delete-users', () => {
  app.extensionData.for('preservemygames-delete-users').registerPermission(
    {
      icon: 'fas fa-user-times',
      label: app.translator.trans('preservemygames-delete-users.admin.permissions.delete_users_label'),
        ...
        name: (
          <label className="PmgUserSelect PmgUserSelect--header">
            <input
              ...
              aria-label={app.translator.trans('preservemygames-delete-users.admin.select_all')}
          ...
          return (
            <label className="PmgUserSelect">
              <input
                ...
                aria-label={app.translator.trans('preservemygames-delete-users.admin.select_user', {
                  username: user.displayName(),
                })}
                onclick={(e) => e.stopPropagation()}
                onchange={(e) => {
    ...
    items.add(
      ...
      <div className="PmgUserBulkBar">
        <span className="PmgUserBulkBar-status">
          {status
            ? status
            : app.translator.trans('preservemygames-delete-users.admin.selected_count', { count })}
        </span>
        <Button
          className="Button Button--danger"
          icon="fas fa-user-times"
        ...
        >
          {app.translator.trans('preservemygames-delete-users.admin.bulk_delete_button', { count })}
        </Button>
      </div>,
      85
    );
  ...
  UserListPage.prototype.pmgDeleteUser = async function (user) {
    ...
    this.pmgBulkStatus = app.translator.trans('preservemygames-delete-users.admin.deleting_one', {
      username: user.displayName(),
    });
    ...
    try {
      ...
      app.store.remove(user);
      this.pmgBulkStatus = app.translator.trans('preservemygames-delete-users.admin.delete_done_one', {
        username: user.displayName(),
      });
      this.isLoadingPage = true;
      await this.loadPage(this.pageNumber);
    } catch (error) {
      this.pmgBulkStatus = app.translator.trans('preservemygames-delete-users.admin.delete_failed');
      throw error;
    } finally {
      this.pmgBulkDeleting = false;
      m.redraw();
  ...
  UserListPage.prototype.pmgBulkDeleteSelected = async function () {
    ...
    const confirmed = window.confirm(
      confirmText(
        app.translator.trans('preservemygames-delete-users.admin.bulk_delete_confirm', {
          count: ids.length,
        })
      )
    );
    ...
    this.pmgBulkDeleting = true;
    this.pmgBulkStatus = app.translator.trans('preservemygames-delete-users.admin.deleting_many', {
      count: ids.length,
    });
    m.redraw();
    ...
    try {
      ...
      if (skipped > 0) {
        this.pmgBulkStatus = app.translator.trans(
          'preservemygames-delete-users.admin.bulk_delete_partial',
          { deleted, skipped }
        );
        app.alerts.show(
          { type: 'warning' },
          app.translator.trans('preservemygames-delete-users.admin.bulk_delete_partial', {
            deleted,
            skipped,
          })
        );
        ...
        this.pmgBulkStatus = app.translator.trans(
          'preservemygames-delete-users.admin.bulk_delete_success',
          { count: deleted }
        );
        app.alerts.show(
          { type: 'success' },
          app.translator.trans('preservemygames-delete-users.admin.bulk_delete_success', {
            count: deleted,
          })
        );
      }
