...
Docker stacks for [Coolify](https://coolify.io/). Each service has a local compose file and a Coolify compose file. Images publish to `ghcr.io/preservemygames/extra`.

## Quick reference

| Service | Folder | Coolify port | Image |
|---------|--------|--------------|-------|
| Flarum | `flarum/` | RavenGuard `8080` (forum), `9090` (guard admin) | `ghcr.io/preservemygames/extra/flarum` + `.../ravenguard` |
...
| cgit | `cgit/` | 8080 | `ghcr.io/preservemygames/extra/cgit` |
| BugPin | `bugpin/` | 7300 | `ghcr.io/preservemygames/extra/bugpin` |
| RavenGuard | `ravenguard/` | built for Flarum (and standalone smoke) | `ghcr.io/preservemygames/extra/ravenguard` |
...
Rootless forum image with bundled PreserveMyGames extensions. Coolify traffic goes through [RavenGuard](https://github.com/Quad4-Software/ravenguard) in behind-proxy mode:
...
RavenGuard keeps its raven logo; challenge branding text comes from `RG_UI_BRAND` (default MeshChatX Forum). Change the RavenGuard admin password after first login.
...
**Maintenance** (`preservemygames-maintenance`)
...
```bash
MEDIAWIKI_LOGO_URL=https://preservemygames.org/static/img/logo.svg
```
...
```bash
COPYPARTY_SITE_NAME=PreserveMyGames Files
```
